...
: stream_(), level(level), line_(line), basename_(filename)
{
    formaitTime();
}
...
void Logger::Impl::formaitTime()
{
    ...
    int n = strftime(str_t, sizeof str_t, "%Y-%m-%d %H:%M:%S.", p_time);
    char buf[10];
    snprintf(buf, sizeof buf, "%06d ", tv.tv_usec);
    strcat(str_t, buf);
    stream_<<str_t<<CurrentThread::tid()<<" "<<LogLevelName[level]<<" ";
}

Logger::Logger(const LogLevel& level, const char* filename, int len) : impl(level, filename, len){}
